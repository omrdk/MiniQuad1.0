 ...
 */
#define MPU9250
#define i2c_write(a, b, c, d) WR_MPU(b, c, d) // -> a :: I2C Slave Address
#define i2c_read(a, b, c, d)  RD_MPU(b, c, d) // -> a :: I2C Slave Address
...
int dak_reset_fifo()
{ unsigned char data = BIT_FIFO_RST;
  return i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &data); }

int dak_dmp_intsts()
{ unsigned char tmp[2];
  i2c_read(st.hw->addr, st.reg->int_status, 1, tmp);
  if(tmp[0]&0x2)return 1; else return 0; }

int dak_read_fifo(unsigned short length, unsigned char *data)
{
  unsigned char tmp[2];
