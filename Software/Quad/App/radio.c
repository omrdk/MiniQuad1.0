...
{ memcpy(rf_rxbuf, rxPacket.payload, RADIO_DATA_SIZE);
  ...
  { m_fl=m_fr=m_bl=m_br=out;            // Tüm motorlarý çalýþtýrmadan önce hepsinin durduðundan emin ol
    ...
    { pid_A.Kp=(*(I16*)&rf_rxbuf[ 8])/100.0f;
      ...
      fZRO=(*(I16*)&rf_rxbuf[18])/100.0f; }       // Referans açý fZR0
    else if(pid_mask==1)
...
{ txPacket.payload[0]=0xDE;
  ...
  I16 r_ynyp=(I16)(pid_R.y*(F32)100);                   // pid_R'nin Max min deðerleri
  txPacket.payload[14] = (U08)((r_ynyp    ) & 0xFF);
  txPacket.payload[15] = (U08)((r_ynyp>> 8) & 0xFF);

  I16 p_ynyp=(I16)(pid_P.y*(F32)100);                   // pid_P'nin Max min deðerleri
